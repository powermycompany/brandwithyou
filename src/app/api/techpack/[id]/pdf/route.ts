...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: design, error: designErr } = await supabase
    ...
    .eq("id", params.id)
    .single();
  if (designErr || !design) return new NextResponse("Not found", { status: 404 });
  if (design.user_id !== user.id) return new NextResponse("Forbidden", { status: 403 });
  ...
  page.drawText("Tech Pack", { x: margin, y: height - margin - 12, size: 20, font: fontBold });
  ...
  line("Design ID", design.id, blockBottomY);
