...
import { updateCustomerProfile } from "@/server/actions/updateCustomerProfile";
...
export default async function CustomerProfilePage() {
  ...
  const { data: p, error } = await supabase
    .from("profiles")
    .select("id,full_name,account_name,email,phone,country,preferred_language,gender,role,status")
    .eq("id", uid)
    .maybeSingle();
  ...
  const { data: favRaw, error: fErr } = await supabase
    .from("customer_favorite_brands")
    .select("brand_id")
    .eq("customer_id", uid);

  if (fErr) throw new Error(fErr.message);

  const defaultFav = (favRaw ?? []).map((r: any) => Number(r.brand_id)).filter((n) => Number.isFinite(n));
    ...
    <div className="row" style={{ flexDirection: "column", gap: 14 }}>
      <div className="card">
        <div className="cardInner">
          ...
          <p className="p">Update your details. Email is managed by authentication.</p>
      ...
      <div className="card">
        <div className="cardInner">
          <form action={updateCustomerProfile}>
            <div className="row customerProfileTopRow">
              <div className="customerProfileField" style={{ flex: "1 1 320px" }}>
            ...
            <div className="row customerProfileSelectRow">
              <div className="customerProfileSelectField" style={{ flex: "0 0 220px" }}>
              ...
              <div className="customerProfileSelectField" style={{ flex: "0 0 260px" }}>
                <label className="p">Gender</label>
                <div className="spacer" style={{ height: 6 }} />
                <select className="input" name="gender" defaultValue={prof.gender ?? "na"}>
            ...
            <BrandMultiSelect
              name="favorite_brand_ids_json"
              brands={brands}
              defaultSelectedIds={defaultFav}
              title="Favorite brands"
              subtitle="Used to personalize your browsing and recommendations later."
            />

            <div className="spacer" />

            <button className="btn btnPrimary customerProfileSaveButton" type="submit">
              Save
            </button>
          </form>
        </div>
